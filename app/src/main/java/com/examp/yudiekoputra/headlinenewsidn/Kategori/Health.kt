...
class Health : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = NewsAdapter(this@Health, list)
        news.setHasFixedSize(true)
        news.setLayoutManager(LinearLayoutManager(this@Health))
    ...
    fun refresh() {
        loading = ProgressDialog(this@Health)
            ...
            ?.enqueue(object : Callback<ResponseNews?> {
                override fun onResponse(
                    ...
                    if (response!!.isSuccessful()) {
                        ...
                        news.setAdapter(NewsAdapter(this@Health, list))
                        adapter?.notifyDataSetChanged()
                    } else {
                        hideDialog()
                        Toast.makeText(this@Health, "Gagal mengambil data !", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
    ...
    override fun onSupportNavigateUp(): Boolean {
        val intent = Intent(this@Health, MainActivity::class.java)
