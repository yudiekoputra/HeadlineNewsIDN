...
class Sport : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = NewsAdapter(this@Sport, list)
        news.setHasFixedSize(true)
        news.setLayoutManager(LinearLayoutManager(this@Sport))
    ...
    fun refresh() {
        loading = ProgressDialog(this@Sport)
            ...
            ?.enqueue(object : Callback<ResponseNews?> {
                override fun onResponse(
                    ...
                    if (response!!.isSuccessful()) {
                        ...
                        news.setAdapter(NewsAdapter(this@Sport, list))
                        adapter?.notifyDataSetChanged()
                    } else {
                        hideDialog()
                        Toast.makeText(this@Sport, "Gagal mengambil data !", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
